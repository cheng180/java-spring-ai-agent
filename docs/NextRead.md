...
> ⚠️ **历史记录**：本文档截至 **2026-07-29**（阶段二 + 阶段三）。此后代码又经历：
> 阶段四混合检索（BM25+RRF）、查询分级分层检索与回复过长治理（#21–#27）、
> 模糊路由回退并入对话理解链路、Langfuse 可观测、macan 接入、销售线索兜底。
> **当前状态请以 `README.md` 与 `docs/智能汽车销售客服系统-整体详细设计.md` 为准**，
> 本文保留作为历史实施记录。

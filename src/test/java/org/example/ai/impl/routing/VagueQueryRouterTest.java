...
class VagueQueryRouterTest {
    ...
    void storeHotReturnsGuide() {
        ...
        when(mockGeoLocator.locate(any()))
                .thenReturn(new org.example.ai.impl.location.GeoLocation(30.28, 120.02, "杭州"));
        when(mockStoreLocator.findNearest(30.28, 120.02))
                .thenReturn(new org.example.ai.impl.location.StoreInfo(1, "杭州店", "S1", "addr", "110", "9-18", "杭州", 30.28, 120.02, true));
        when(mockHotCarRepo.getHotCars(1, 3))
                .thenReturn(java.util.List.of(
                        new org.example.ai.impl.location.HotCar("宋PLUS DM-i", 30, 20),
                        new org.example.ai.impl.location.HotCar("Model Y", 25, 15),
                        new org.example.ai.impl.location.HotCar("理想L6", 20, 10)));

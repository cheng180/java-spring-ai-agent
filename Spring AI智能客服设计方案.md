...
public class StoreLocator {
    ...
    /**
     * 根据经纬度找最近门店（Haversine公式）
     */
    public StoreInfo findNearest(double userLat, double userLng) {
        // Haversine 公式在 SQL 中计算
        String sql = """
            SELECT *, 
        ...
        return jdbc.queryForObject(sql, new StoreInfoRowMapper(), 
            userLat, userLng, userLat);
    }
    
    /**
     * 从用户 IP 获取大致位置（调用免费IP定位API）
     */
    public GeoLocation getLocationFromIp(String userIp) {
        // 调用 ip-api.com 等免费服务
        // 或百度/高德 IP 定位 API
    }
}
